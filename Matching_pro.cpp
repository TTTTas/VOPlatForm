...
EpipolarGeometry_pro::EpipolarGeometry_pro(QString path)
    : Project_Base(path)
{
    // 额外的初始化可以在这里完成
    ...
    QString temp_line = nextMatchingLine();
    inputfolder = parentDir + temp_line;

    temp_line = nextMatchingLine();
...
QString EpipolarGeometry_pro::matchingName() const
{
    return getPro_Name();  // 从基类继承的项目名称
}

QString EpipolarGeometry_pro::nextMatchingLine()
{
    return next();  // 调用基类中的 next() 方法
}

void EpipolarGeometry_pro::update(QString path1, QString path2, QString path3)
{
    inputfolder = path1;
    outputfolder = path2;
    resultpath = path3;
...
void EpipolarGeometry_pro::save()
{
